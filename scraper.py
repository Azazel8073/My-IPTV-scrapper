...
# Dynamically loads the long string out of the GitHub secrets environment variables pool
SUBREDDIT_RSS_URL = os.environ.get("TARGET_RSS_STREAM")
...
def main():
    if not SUBREDDIT_RSS_URL:
        print("Error: TARGET_RSS_STREAM variable is blank or missing in the GitHub environment.")
        return

    print(f"Connecting to target endpoint via pipeline environment: {SUBREDDIT_RSS_URL}")
